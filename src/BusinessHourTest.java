...
public class BusinessHourTest {
    ...
    public void testWeekends() {
        ...
        assertEquals(0, BusinessHour.getMinsByDate(saturday));
    }
    ...
    public void testWeekenddays() {
        ...
        assertEquals(SECONDS_OF_ONE_WORKING_DAY, BusinessHour.getMinsByDate(saturday));
    }
    ...
    public void testHoliday() {
        ...
        assertEquals(0, BusinessHour.getMinsByDate(newYearsDay));
    }
    ...
    public void testInWorkingDate() {
        ...
        assertEquals(SECONDS_OF_ONE_WORKING_DAY, BusinessHour.getMinsByDateTime(begin, null));

        // 근무 시작과 근무 종료 사이 등록 된 문의
        int regHour = 15;
        begin = LocalDateTime.of(2018, 1, 5, regHour, 0, 0);
        assertEquals((BusinessHour.END_WORK_HOUR - regHour) * MINUTES_OF_ONE_HOUR * SECONDS_OF_ONE_MINUTE, BusinessHour.getMinsByDateTime(begin, null));

        // 근무 시작 이후에 등록된 문의
        begin = LocalDateTime.of(2018, 1, 5, 20, 0, 0);
        assertEquals(0, BusinessHour.getMinsByDateTime(begin, null));

        // 근무일에 종료된 문의
        // 근무 시작 전 종료 된 문의
        LocalDateTime end = LocalDateTime.of(2018, 1, 5, 8, 0, 0);
        assertEquals(0, BusinessHour.getMinsByDateTime(null, end));

        // 근무 시작과 근무 종료 사이 종료 된 문의
        regHour = 15;
        end = LocalDateTime.of(2018, 1, 5, regHour, 0, 0);
        assertEquals((regHour - BusinessHour.BEGIN_WORK_HOUR) * MINUTES_OF_ONE_HOUR * SECONDS_OF_ONE_MINUTE, BusinessHour.getMinsByDateTime(null, end));

        // 근무 시작 이후에 종료된 문의
        end = LocalDateTime.of(2018, 1, 5, 20, 0, 0);
        assertEquals(SECONDS_OF_ONE_WORKING_DAY, BusinessHour.getMinsByDateTime(null, end));
    }
    ...
    public void testInWorkingDateTime() {
        ...
        assertEquals(30 * SECONDS_OF_ONE_MINUTE - 45, BusinessHour.getMinsByDateTime(begin, null));
        ...
        assertEquals(20 * SECONDS_OF_ONE_MINUTE + 35, BusinessHour.getMinsByDateTime(null, end));
